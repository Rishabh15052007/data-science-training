...
# ------------------------------------
# 1. FORCED PREMIUM LIGHT CANVAS SYSTEM
# ------------------------------------
st.set_page_config(
...
# Overriding dark modes to force a crisp, ultra-clean professional look
st.markdown("""
    <style>
    ...
    /* Clean white analytical blocks for metrics */
    .metric-card {
...
# ------------------------------------
# 4. DASHBOARD HEADER & SUMMARY MATRIX
# ------------------------------------
...
if not filtered_df.empty:
    ...
    # ------------------------------------
    # 5. THE 8 DISTINCT HIGH-CONTRAST DIAGRAMS
    # ------------------------------------
    ...
    with r1_left:
        ...
        # Vibrant diverse color sequence for individual slices
    ...
    with r1_right:
        ...
        # Bold Royal Blue line for timeline analysis
    ...
    with r2_left:
        ...
        # Modern corporate teal color for bars
    ...
    with r2_right:
        ...
        # Multi-colored dot configurations categorized by Series
    ...
    # --- ROW 3: Histogram & Area ---
    r3_left, r3_right = st.columns(2)
    with r3_left:
        st.markdown("### 5. 📊 Distribution of Product Unit Prices")
        # Deep plum colored distribution bars
        fig5 = px.histogram(filtered_df, x='Unit Price (USD)', nbins=15, template='plotly_white',
                             color_discrete_sequence=['#701a75'])
        st.plotly_chart(fig5, use_container_width=True)
        
    with r3_right:
        st.markdown("### 6. 📈 Cumulative Volume Shipped Trajectory")
        vol_data = filtered_df.groupby('Date')['Units Sold'].sum().reset_index()
        # Clean soft amber filled area tracking volume growth
        fig6 = px.area(vol_data, x='Date', y='Units Sold', template='plotly_white',
                       color_discrete_sequence=['#f59e0b'])
        st.plotly_chart(fig6, use_container_width=True)

    st.write("---")

    # --- ROW 4: Box Plot & Funnel ---
    r4_left, r4_right = st.columns(2)
    with r4_left:
        st.markdown("### 7. 📦 Thermal TDP Dispersion Across Market Segments")
        # Balanced clean emerald green box plot showing statistical spread
        fig7 = px.box(filtered_df, x='Segment', y='TDP (Watts)', points="all", template='plotly_white',
                      color_discrete_sequence=['#10b981'])
        st.plotly_chart(fig7, use_container_width=True)
        
    with r4_right:
        st.markdown("### 8. 🌪️ Volumetric Product Demand Pipeline by Segment")
        funnel_data = filtered_df.groupby('Segment')['Units Sold'].sum().reset_index().sort_values(by='Units Sold', ascending=False)
        # Gradient corporate corporate blue pipeline blocks
